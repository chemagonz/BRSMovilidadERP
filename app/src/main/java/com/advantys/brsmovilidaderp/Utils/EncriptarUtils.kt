...
class EncriptarUtils {
    ...
    private fun crc16(data: ByteArray, crc: Short): Short {
        ...
        for (i in data.indices) {
            crcLocal = (CRC_16[((crcLocal.toInt() ushr 8) xor data[i].toInt()) and 0xff] xor ((crcLocal.toInt() shl 8).toShort())).toShort()
        }
        return crcLocal
    }
    ...
    private fun crc32(data: ByteArray, crc: Int): Int {
        ...
        for (i in data.indices) {
            crcLocal = CRC_32[(crcLocal xor data[i].toInt()) and 0xff] xor ((crcLocal ushr 8) and 0xffffff)
        }
        return crcLocal.inv()
    }
    ...
    fun crc32(data: Byte, length: Int, crc: Int): Int {
        var crcLocal = crc.inv()
        repeat(length) {
            crcLocal = CRC_32[(crcLocal xor data.toInt()) and 0xff] xor ((crcLocal ushr 8) and 0xffffff)
        }
        return crcLocal.inv()
    }
    ...
    private fun precalc16() {
        ...
        for (i in 0 until 256) {
            ...
            for (k in 0 until 8) {
                crc = ((crc.toInt() shl 1) xor if ((crc.toInt() and 0x8000) != 0) 0x1021 else 0).toShort()
            }
